...
<body>
	...
	<script type="text/javascript">
		...
		function page(index) {
			...
			$.post("<%=path%>/manager/shortvideo_list",{'page':index,'count':pageSize,'status':2,'user_id':user_id,'nick_name':nick_name},function(result){
      ...
      function reviewTableTr(pageData,tr) {
			 ...
			 //操作单元格
			  ...
			  //操作单元格
			  toAdd+="<a class='button border-red' href='javascript:void(0)'	onclick='return changeVideoStatus("+id+",0)'><span class='icon-edit'></span>移除违规</a>";
			  toAdd+="<a class='button border-red' href='javascript:void(0)'	onclick='return changeVideoStatus("+id+",4)'><span class='icon-edit'></span>删除</a>";
			  toAdd+="</div></td></tr>";
			 tr.after(toAdd);
		}
