...
<body>
		...
		<div class="panel admin-panel">
			...
			<div class="padding border-bottom">
				<ul class="search">
				 ...
				 <li>
			        <input type="text" placeholder="请输入动态id" name="dy_id" class="input" style="width:150px; line-height:17px;display:inline-block" />
                         <a href="javascript:void(0)" class="button border-main icon-search" onclick="doSearch()" >搜索</a>
                   </li> 
	...
	<script type="text/javascript">
		...
		function page(index) {
			...
			$.post("<%=path%>/manager/unselected_dynamic_list",{'pageIndex':index,'nick_name':nickName,'dy_id':dy_id,'user_id':user_id},function(result){
		...
		function add(id) {
				$.post("<%=path%>/manager/add_dy_to_selected",{id:id,currentPage:currentPage,'nick_name':nickName,'user_id':user_id,'dy_id':dy_id},function(result){
		...
		function del(id) {
			$.post("<%=path%>/manager/dynamic_del",{id:id,currentPage:currentPage,'nick_name':nickName,'user_id':user_id,'dy_id':dy_id},function(result){
		        $("#tr_"+id).remove();//移除当前的元素
		...
		function illegal(id) {
			...
			$.post("<%=path%>/manager/illegal_unselected",{id:id,currentPage:currentPage,'nick_name':nickName,'user_id':user_id,'dy_id':dy_id},function(result){
		        $("#tr_"+id).remove();//移除当前的元素
      ...
      function ignore(id) {
			$.post("<%=path%>/manager/ignore_unselected",{id:id,currentPage:currentPage,'nick_name':nickName,'user_id':user_id,'dy_id':dy_id},function(result){
		        $("#tr_"+id).remove();//移除当前的元素
      ...
      function reviewTableTr(pageData,tr) {
			...
			// var device=parent.getDeviceTxt(pageData['_from']);
			 ...
			 toAdd+="<td><div class='button-group'><a class='button border-green' href='javascript:void(0)'	onclick='return add("+pageData["id"]+")'><span class='icon-plus-square-o'></span>添到首页</a><a class='button border-yellow' href='javascript:void(0)'	onclick='return illegal("+pageData["id"]+")'><span class='icon-trash-o'></span>违规</a></div></td>";
			 toAdd+="</tr>";
			 tr.after(toAdd);
		}
