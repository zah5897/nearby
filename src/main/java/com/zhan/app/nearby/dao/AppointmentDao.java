...
public class AppointmentDao extends BaseDao<Appointment> {
	...
	public List<Appointment> queryAll(long user_id, Integer last_id, int count) {
		...
		String sql = "select a.*,u.nick_name,u.sex,u.lat,u.lng,c.name as city_name from " + getTableName()
				+ " a left join t_user u on a.uid=u.user_id left join t_sys_city c on a.city_id=c.id  where a.id<? order by a.id desc limit ?";
	...
	private static BeanPropertyRowMapper<Appointment> appointmentMapper = new BeanPropertyRowMapper<Appointment>(Appointment.class) {
		@Override
