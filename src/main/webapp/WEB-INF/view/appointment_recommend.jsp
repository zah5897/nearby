      ...
      function reviewTableTr(pageData,tr) {
	    ...
	    function show(img){
 	    ...
 	    function changestatus(id,newStatus){
			$.post("<%=path%>/manager/changeAppointMentStatus",{'user_id':user_id,'nick_name':nick_name,'id':id,'status':6,'page':currentPageIndex,'count':pageSize,'to_state':newStatus},function(result){
				 var json=JSON.parse(result);
			        if(json.code==0){
			        	$("table tr[id*='tr_'").each(function(i){
				        	this.remove();//移除当前的元素
