...
<body>
	...
	<script type="text/javascript">
		...
		function page(index) {
			...
			$.post("<%=path%>/manager/appointment_list",{'page':index,'count':pageSize,'status':1,'user_id':user_id,'nick_name':nick_name},function(result){
      ...
      function reviewTableTr(pageData,tr) {
			 ...
			 //操作单元格
			  ...
			  //操作单元格
			  toAdd+="<a class='button border-red' href='javascript:void(0)'	onclick='return changestatus("+id+",6)'><span class='icon-edit'></span>推荐</a>";
	    ...
	    function show(img){
 	    ...
 	    function changestatus(id,newStatus){
			$.post("<%=path%>/manager/changeAppointMentStatus",{'user_id':user_id,'nick_name':nick_name,'id':id,'status':1,'page':currentPageIndex,'count':pageSize,'to_state':newStatus},function(result){
				 var json=JSON.parse(result);
			        if(json.code==0){
			        	$("table tr[id*='tr_'").each(function(i){
				        	this.remove();//移除当前的元素
