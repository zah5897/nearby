...
<body>
	...
	<script type="text/javascript">
		...
		function del(id) {
				$.post("<%=path%>/manager/removes_illegal_dynamic",{id:id,currentPage:currentPage},function(result){
		...
		function backToCheck(id) {
				$.post("<%=path%>/manager/backToCheck",{id:id,currentPage:currentPage},function(result){
			        $("#tr_"+id).remove();//移除当前的元素
      ...
      function reviewTableTr(pageData,tr) {
			 ...
			 toAdd+="<td><div class='button-group'><a class='button border-main' href='javascript:void(0)'	onclick='return backToCheck("+pageData["id"]+")'><span class='icon-trash-o'></span>恢复为待审核</a><a class='button border-red' href='javascript:void(0)'	onclick='return del("+pageData["id"]+")'><span class='icon-trash-o'></span>删除</a></div></td>";
			 toAdd+="</tr>";
			 tr.after(toAdd);
		}
		...
		$("#del_batch").click(function() {
			...
			if(chk_value.length>0){
					...
					$.post("<%=path%>/manager/removes_illegal_dynamics", {
							ids : ids,
